...
function addChannelEquipment(data) {
  ...
  // 先清理现有的源和图层
  if (map.getSource("channel-points")) {
    map.removeSource("channel-points");
  }
  if (map.getLayer("channel-points-layer")) {
    map.removeLayer("channel-points-layer");
  }
  ...
  Object.entries(iconMap).forEach(([key, url]) => {
    ...
    img.onload = () => {
      ...
      // console.log("poi_111111111",map.hasImage("poi_" + key), img)
      loadedCount++;
